...
function checkUserName(un) {
    ...
    $('#username').css({
        'border-bottom': '1px solid #' + (isUserNameOk ? '505050' : 'f02020')
    });
...
function setTitleState(blink) {
    ...
    switch (+sariaSettings.titleNotificationLevel) {
        case 0:
            break;
        case 1:
            titleText = '* Тебе написали *';
            break;
        case 2:
            titleText = '* Новое сообщение *';
            break;
        default:
            saria_error('BAD SETTING');
    }
...
function saria_init(auth) {
    ...
    $('#chat-settings-overlay').mousewheel(function(ev, d) {
        ev.preventDefault;
        return false;
    });
    $('#chat-settings').mousewheel(function(ev, d) {
        ev.preventDefault;
        return false;
    });
    ...
    $(window).focus(function() {
            sariaStatus.isFocused = true;
            setTitleState(false)
        })
        .blur(function() {
            sariaStatus.isFocused = false
        });
...
function saria_connect() {
    ...
    try {
        ...
        sariaSocket.onmessage = function(ev) {
            saria_getCommand(ev.data);
        };
        sariaSocket.onclose = function(ev) {
            saria_error(ev.code, (ev.reason) ? ev.reason : 'Websocket disconnected itself');
        };
    } catch (e) {
